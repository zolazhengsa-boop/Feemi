const API_BASE = "https://feemi-api.vercel.app/api/v1";
const $ = id => document.getElementById(id);
const icons = {"AI工具":"🤖","生活服务":"🏠","娱乐影音":"▶️","金融理财":"💳"};
let filter = "全部", editId = null;
let subs = [];

// ── Field mappers: translate between API format and frontend format ──
const CURRENCY_SYMBOL = { GBP: "£", USD: "$" };
const CURRENCY_CODE = { "£": "GBP", "$": "USD", "€": "GBP", "¥": "GBP" };

function apiSubToFrontend(s) {
  const date = s.next_bill_date ? new Date(s.next_bill_date + "T00:00:00") : null;
  return {
    id: s.id,
    name: s.name,
    price: (s.amount || 0) / 100,
    currency: CURRENCY_SYMBOL[s.currency] || "£",
    category: s.category || "生活服务",
    day: date ? date.getDate() : 1,
    month: (s.cycle === "annual" && date) ? date.getMonth() + 1 : "",
    cycle: s.cycle === "weekly" ? "monthly" : (s.cycle || "monthly"),
    status: s.is_active ? "使用中" : "已取消",
    note: ""
  };
}

function frontendSubToApi(data) {
  let next_bill_date = null;
  if (data.day) {
    const now = new Date();
    const year = now.getFullYear();
    const month = (data.cycle === "annual" && data.month) ? Number(data.month) : now.getMonth() + 1;
    const d = String(Number(data.day)).padStart(2, "0");
    const m = String(month).padStart(2, "0");
    next_bill_date = `${year}-${m}-${d}`;
  }
  return {
    name: data.name,
    amount: Math.round((data.price || 0) * 100),
    currency: CURRENCY_CODE[data.currency] || "GBP",
...
// ── Load subscriptions from live API ──
async function loadSubs() {
  try {
    const r = await fetch(`${API_BASE}/subscriptions`);
    const d = await r.json();
    subs = (d.data || []).map(apiSubToFrontend);
  } catch(e) {
...
function save() {
  localStorage.setItem("feemi-memory-v7", JSON.stringify(memory));
  localStorage.setItem("feemi-secrets-v7", JSON.stringify(saved));
}
...
async function saveSub() {
  let data = {
    ...
    month: $("subMonth").value,
    status: $("subStatus").value,
    note: $("subNote").value
  };
  if (data.cycle === "annual" && !data.month) data.month = new Date().getMonth() + 1;
  const apiData = frontendSubToApi(data);
  try {
    if (editId) {
      await fetch(`${API_BASE}/subscriptions/${editId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...apiData, is_active: data.status !== "已取消" })
      });
    } else {
...
async function delSub() {
  if (editId && confirm("确定删除这个订阅吗？")) {
    try {
      await fetch(`${API_BASE}/subscriptions/${editId}`, { method: "DELETE" });
    } catch(e) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("addTop").onclick = () => openSheet();
  $("addBottom").onclick = () => openSheet();
  $("close").onclick = closeSheet;
  $("sheet").onclick = e => { if (e.target.id === "sheet") closeSheet() };
  $("saveSub").onclick = saveSub;
  $("deleteSub").onclick = delSub;
  $("search").oninput = renderBills;
  document.querySelectorAll(".chip").forEach(c => c.onclick = () => { filter = c.dataset.filter; document.querySelectorAll(".chip").forEach(x => x.classList.remove("active")); c.classList.add("active"); renderBills() });
  $("send").onclick = () => { let m = $("message").value.trim(); if (m) askFeebi(m) };
  $("message").addEventListener("keydown", e => { if (e.key === "Enter") { let m = $("message").value.trim(); if (m) askFeebi(m) } });
  document.querySelectorAll("[data-question]").forEach(b => b.onclick = () => askFeebi(b.dataset.question));
  $("saveMemory").onclick = () => { memory = { fav: $("fav").value, worry: $("worry").value, habit: $("habit").value, goal: $("goal").value }; save(); renderMemory(); alert("Feebi 记住了 💜") };
  $("exportData").onclick = async () => { let text = JSON.stringify({ subs, memory, saved }, null, 2); try { await navigator.clipboard.writeText(text); alert("已复制备份") } catch { alert(text) } };
  $("resetData").onclick = () => { if (confirm("清空数据？此操作不可撤销。")) { loadSubs() } };
  $("clearSaved").onclick = () => { saved = []; save(); renderSaved() };
  loadSubs();
});
